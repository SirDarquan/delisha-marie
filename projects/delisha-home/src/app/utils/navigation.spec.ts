...
describe('getKitchenUrl', () => {
  it('should return http://localhost:4200/kitchen in dev mode on localhost', () => {
    expect(getKitchenUrl({ hostname: 'localhost', isDev: true })).toBe(
      'http://localhost:4200/kitchen',
    );
    expect(getKitchenUrl({ hostname: '127.0.0.1', isDev: true })).toBe(
      'http://localhost:4200/kitchen',
    );
  });
  ...
  it('should fallback to window.location.hostname when hostname option is omitted', () => {
    ...
    expect(result.endsWith('/kitchen')).toBe(true);
  });
});
